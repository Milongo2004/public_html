...
$hasta=$_GET["hasta"];
 ...
 $query=mysqli_query($conexion,"SELECT rotulos2.*,referencias2.`nombre` AS 'referencia', colores2.`nombre` AS 'Color', pedidos2.`codigoP` AS Pedido, lotes2.`nombreL` AS Lote, estaciones2.`nombre` AS 'estacionActual' FROM rotulos2 INNER JOIN referencias2 ON rotulos2.`referenciaId`= referencias2.`id` INNER JOIN colores2 ON rotulos2.`colorId` = colores2.`id` INNER JOIN pedidos2 ON rotulos2.`pedido` = pedidos2.`idP` INNER JOIN lotes2  ON  rotulos2.`loteId`= lotes2.`id` INNER JOIN estaciones2 ON rotulos2.`estacionId2` = estaciones2.`id`  WHERE Fecha BETWEEN '$desde' AND '$hasta'"); 
...
<div><h1>FECHA DE BUSQUEDA TABLA ROTULOS de <?php echo $desde;?>&nbsp;&nbsp;A&nbsp;&nbsp;<?php  echo $hasta  ?></h1></div>
<br><br>
